...
for (let i = 0; i < 20; i++) {
  ...
  const clicked = await page.evaluate(() => {
    ...
    // Try buttons with priority order
    ...
    // Click the Preview step link directly
    const links = document.querySelectorAll('[class*="step"], [class*="Step"]');
    for (const l of links) {
      if (l.textContent.includes('Preview')) { l.click(); return 'preview step'; }
    }
    return null;
  });
...
// Screenshot
await page.screenshot({ path: 'C:/Temp/Sheetlab/debug-screenshot.png', fullPage: true });
console.log('Screenshot saved');

// PDF
await page.pdf({
  path: 'C:/Temp/Sheetlab/debug-print.pdf',
  format: 'A4',
